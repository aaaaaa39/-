...
void CheckMapPlayer(PLAYER* player)
{
	if (player->pos.x < MAP_LEFT) {
		player->pos.x = MAP_LEFT;
		player->spd = 0.0f; // 移動を停止
	}
	if (player->pos.x > MAP_RIGHT) {
		player->pos.x = MAP_RIGHT;
		player->spd = 0.0f;
	}
	if (player->pos.z < MAP_DOWN) {
		player->pos.z = MAP_DOWN;
		player->spd = 0.0f;
	}
	if (player->pos.z > MAP_TOP) {
		player->pos.z = MAP_TOP;
		player->spd = 0.0f;
	}
}
...
HRESULT InitPlayer(void)
{
	...
	g_Player.pos = XMFLOAT3(0.0f, PLAYER_OFFSET_Y /* + 50.0f */ , -400.0f);
...
void UpdatePlayer(void)
{
	...
	g_Player.rot.y = cam->rot.y;	// プレイヤーが向いた方向にカメラを向ける
	// Key入力があったら移動処理する
	if (g_Player.spd > 0.0f)
	{
		// 入力のあった方向へプレイヤーを向かせて移動させる
		g_Player.pos.x += sinf(g_Player.dir + cam->rot.y) * g_Player.spd;
		g_Player.pos.z += cosf(g_Player.dir + cam->rot.y) * g_Player.spd;
	}

	// MAPの境界と制限
	CheckMapPlayer(&g_Player);
	...
	MODE* mode = GetModeUi();
	if (mode->use == 0) {
		if (GetKeyboardTrigger(DIK_SPACE) && (g_Player.ammo != 1)) {
			XMFLOAT3 pos = g_Player.pos;
	...
	{
		if (GetKeyboardPress(DIK_SPACE) && (g_Player.ammo != 1)) {
			XMFLOAT3 pos = g_Player.pos;
			SetBullet(pos, g_Player.rot);
			g_Player.ammo += 0.02f;
			//PlaySound(SOUND_LABEL_SE_shoot);
	...
	if (mode->use == 0) {
		if (IsButtonPressed(0, BUTTON_R2) && (g_Player.ammo != 1)) {
			XMFLOAT3 pos = g_Player.pos;
			SetBullet(pos, g_Player.rot);
			g_Player.ammo += 0.1f;
			//PlaySound(SOUND_LABEL_SE_shoot);
...
//
//		// 計算したクォータニオンの回転を5％ずつ変更していく
//		// いきなり求めた角度を変更すると、がたがたした動きになる
...
void DrawPlayer(void)
{
	...
	for (int i = 0; i < PLAYER_PARTS_MAX; i++)
	{
		...
		// 使われているなら処理する
		if (g_Parts[i].use == TRUE) continue;
