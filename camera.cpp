...
void UpdateCamera(void)
{
	...
	const float MIN_ROT_X = -XM_PI / 2; // 最小角度（-90度）
	const float MAX_ROT_X = XM_PI / 2;  // 最大角度（90度）
	const float ROTATE_SPEED = 0.025f;   // 回転速度
	...
	{// 視点旋回「上」
		g_Camera.rot.x += ROTATE_SPEED;
		if (g_Camera.rot.x < MIN_ROT_X)
		{
			g_Camera.rot.x = MIN_ROT_X; // 最小角度に制限
		}
	}
	...
	{// 視点旋回「下」
		g_Camera.rot.x -= ROTATE_SPEED;
		if (g_Camera.rot.x > MAX_ROT_X)
		{
			g_Camera.rot.x = MAX_ROT_X; // 最大角度に制限
		}
	}

	//g_Camera.rot.z = 0.0f; // 不要な回転をリセット
...
void SetCameraAT(XMFLOAT3 pos)
{
	// カメラの注視点を引数の座標にしてみる
	g_Camera.pos = pos;
	g_Camera.pos.y += 30.0f;	// カメラの位置を少し上にする

	// カメラの視点をカメラのY軸回転に対応させている
	g_Camera.at.x = g_Camera.pos.x - sinf(g_Camera.rot.y) * g_Camera.len;	// atとposを反対にしてFPS視点にする
	g_Camera.at.z = g_Camera.pos.z - cosf(g_Camera.rot.y) * g_Camera.len;
