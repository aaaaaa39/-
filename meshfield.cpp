...
// 波の処理
static XMFLOAT3 g_keepwavepos;	// 波の高さを保存
...
void UpdateMeshField(void)
{
	...
	moveX = ( Player->pos.x- g_posField.x) * 0.05f;
	moveZ = (Player->pos.z - g_posField.z) * 0.05f;
	g_posField.x += moveX;
	g_posField.z += moveZ;
...
//
//			// 三角ポリゴンだから２枚分の当たり判定
//			BOOL ans = RayCast(p0, p2, p1, start, end, HitPosition, Normal);	// pは頂点 頂点3つ分
//			if (ans)
//			{
//				return TRUE;
//			}
...
XMFLOAT3 GetWavefield(void)		// 波の情報を取得
{
	return g_keepwavepos;		// 保存した波の高さを渡す
