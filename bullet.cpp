...
#define	MODEL_BULLET		"data/MODEL/bullet.001.obj"			// 読み込むモデル名
...
static DX11_MODEL g_BulletModel;		// 弾のモデルデータ

static float g_ShotCooldown = 0.0f;		 // 発射間隔のクールダウン時間
...
float GetDeltaTime()
{
	static auto lastTime = std::chrono::steady_clock::now(); // 前のフレームの時間を記録
	auto currentTime = std::chrono::steady_clock::now();     // 今の時間を取得
	std::chrono::duration<float> elapsedTime = currentTime - lastTime; // 経過時間を計算
	lastTime = currentTime; // 次のフレームのために現在の時間を記録
	return elapsedTime.count(); // 経過時間を秒単位で返す
}
...
{	LoadModel(MODEL_BULLET, &g_BulletModel);
	...
	{
		ZeroMemory(&g_Bullet[i].material, sizeof(g_Bullet[i].material));
		g_Bullet[i].material.Diffuse = { 1.0f, 1.0f, 1.0f, 1.0f };

		g_Bullet[i].pos = { 0.0f, 0.0f, 0.0f };
		g_Bullet[i].rot = { XM_PI, 0.0f, 0.0f };
		g_Bullet[i].scl = { 1.0f, 1.0f, 1.0f };
		g_Bullet[i].spd = BULLET_SPEED;
		g_Bullet[i].fWidth = BULLET_WIDTH;
		g_Bullet[i].fHeight = BULLET_HEIGHT;
...
{
	// クールダウンの時間を減らす
	if (g_ShotCooldown > 0.0f) {
		g_ShotCooldown -= deltaTime;
	}
...
void DrawBullet(void)
{
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		if (g_Bullet[i].use)
		{
			...
			// 回転を反映
			mtxRot = XMMatrixRotationRollPitchYaw(XM_PI*0.0f, XM_PI*1.0f, XM_PI*1.0f);
...
int SetBullet(XMFLOAT3 pos,XMFLOAT3 rot)
{
	...
	for (int i = 0; i < MAX_BULLET; i++) {
		if (g_Bullet[i].use == FALSE) {
			g_Bullet[i].use = TRUE;
			g_Bullet[i].pos = pos;
			g_Bullet[i].dir = GetDeltaMove(camera->dir, camera->pos);
			g_Bullet[i].rot.y = rot.y;
			g_Bullet[i].rot.x = GetCameraPitch(camera->dir, camera->pos);
			g_Bullet[i].cnt = 240;
			
			nIdxBullet = i;
			g_ShotCooldown = 0.5f;		// 0.5秒で発射可能
